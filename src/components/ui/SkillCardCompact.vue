<template>
  <div class="skill-card-compact group rounded-xl md:rounded-2xl overflow-hidden cursor-pointer" @click="handleClick">
    <div class="p-3 md:p-6 relative">
      <div class="flex items-start gap-2 md:gap-3 mb-2 md:mb-4">
        <div class="w-8 h-8 md:w-14 md:h-14 rounded-lg md:rounded-xl bg-gradient-to-br from-[var(--neon-cyan)] to-[var(--neon-purple)] flex items-center justify-center text-white text-sm md:text-2xl font-black font-mono shadow-lg group-hover:scale-110 transition-transform duration-300 flex-shrink-0">
          {{ skill.name.charAt(0).toUpperCase() }}
        </div>
        <div class="flex-1 min-w-0">
          <h3 class="text-sm md:text-xl font-bold mb-1 md:mb-2 text-[var(--text-light)] group-hover:text-[var(--neon-cyan)] transition-colors line-clamp-1">
            {{ skill.name }}
          </h3>
          <span class="text-[var(--neon-yellow)] text-[10px] md:text-xs font-mono uppercase tracking-wider">
            [{{ getSourceLabel(skill.source.type) }}]
          </span>
        </div>
      </div>
      
      <p class="text-[11px] md:text-sm text-[var(--text-muted)] mb-2 md:mb-3 line-clamp-2 leading-relaxed">
        {{ skill.description }}
      </p>
      
      <div class="flex flex-wrap gap-1 md:gap-2 mb-2 md:mb-3">
        <span v-for="tag in displayedTags" :key="tag" class="px-1.5 md:px-3 py-0.5 md:py-1 bg-[var(--neon-purple)]/20 border border-[var(--neon-purple)]/30 text-[var(--neon-purple)] rounded-full text-[10px] md:text-xs font-mono">
          #{{ tag }}
        </span>
