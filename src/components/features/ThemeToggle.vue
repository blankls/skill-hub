<template>
  <button
    class="relative w-36 h-12 rounded-full cursor-pointer transition-all duration-300 overflow-hidden shadow-lg"
    :class="themeStore.isDark ? 'bg-[#141b3a] border border-[var(--neon-cyan)]' : 'bg-white border border-gray-200'"
    @click="themeStore.toggleTheme()"
  >
    <!-- Background labels -->
    <div class="absolute inset-0 flex items-center justify-between px-4 font-mono text-sm font-bold">
    ...
    <div
      ...
      :style="{
        transform: themeStore.isDark ? 'translateX(92px)' : 'translateX(0)',
